<?php
	...
	class DatabaseFactory extends \PDO {
		...
		public function single(string $query, array $parameters = []) : mixed {
			return $this->parseJSON($this->query($query, null, $parameters)->fetch(\PDO::FETCH_OBJ));
		}
		...
		public function couldBeJSON($value) {
			if(!is_string($value)) {
				return false;
			}
		...
		public function parseJSON($query) {
			if($query) {
				foreach($query AS $index => $object) {
					if(is_array($object) || is_object($object)) {
						foreach($object AS $name => $value) {
							try {								
								if($this->couldBeJSON($value)) {
									$json = json_decode($value, false, 512, JSON_THROW_ON_ERROR | JSON_FORCE_OBJECT);
						...
						try {
							if($this->couldBeJSON($object)) {
								$json = json_decode($object, false, 512, JSON_THROW_ON_ERROR | JSON_FORCE_OBJECT);
		...
		public function fetch(string $query, array $parameters = []) : array | false {
			return $this->parseJSON($this->query($query, null, $parameters)->fetchAll(\PDO::FETCH_OBJ));
		}
		
		public function update(string $table, string | array $where, array $parameters = []) : \PDOStatement | false {
			$fields = '';
			...
			foreach($parameters AS $name => $value) {
				if($value === 'NOW()') {
					$parameters[$name] = date('Y-m-d H:i:s', time());
				}
			}
			
			return $this->query($query, null, $parameters);
		}
