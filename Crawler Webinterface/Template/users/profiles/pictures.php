	...
	<div class="d-flex flex-column flex-grow-1 pb-3" style="height: calc(100% - 200px)">
		 <div class="card d-flex flex-column flex-grow-1 h-100">
			<div class="card-header">
				Bilder (<?php print number_format($total, 0, ',', '.'); ?>)
			</div>
					...
					<?php
						if(empty($pictures)) {
							...
							foreach($pictures AS $picture) {
								$template->display('entry/picture', [
									'picture' => $picture
								]);
							}
							print('</div>');
							...
							<nav>
								<ul class="pagination justify-content-center">
									<li class="page-item <?php print ($current <= 1 ? 'disabled' : ''); ?>">
										<a class="page-link" href="<?php print $this->url('/users/profiles/pictures/' . ($current - 1), $query); ?>" aria-label="Zurück">
									...
									<?php
										...
										foreach($displayed as $i) {
											...
											<li class="page-item <?php print(($i == $current) ? 'active' : ''); ?>">
												<a class="page-link" href="<?php print $this->url('/users/profiles/pictures/' . $i, $query) ?>">
													<?php print $i ?>
												</a>
											</li>
											<?php
									...
									<li class="page-item <?php print ($current >= $pages ? 'disabled' : ''); ?>">
										<a class="page-link" href="<?php print $this->url('/users/profiles/pictures/' . ($current + 1), $query); ?>" aria-label="Weiter">
