	...
	<div class="d-flex flex-column flex-grow-1 pb-3" style="height: calc(100% - 200px)">
		 <div class="card d-flex flex-column flex-grow-1 h-100">
			<div class="card-header">
				Benutzer (<?php print number_format($total, 0, ',', '.'); ?>)
			</div>
				...
				<div class="flex-grow-1 h-100 overflow-auto d-flex flex-column p-3 m-0">
					<?php
						if(empty($users)) {
							...
							print('<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-6 g-4">');
							foreach($users AS $user) {
								$template->display('entry/user', [
									'user' => $user
								]);
							}
							print('</div>');
							...
							<nav>
								<ul class="pagination justify-content-center">
									<li class="page-item <?php print ($current <= 1 ? 'disabled' : ''); ?>">
										<a class="page-link" href="<?php print $this->url('/users/' . ($current - 1), $query); ?>" aria-label="Zurück">
									...
									<?php
										...
										foreach($displayed as $i) {
											...
											<li class="page-item <?php print(($i == $current) ? 'active' : ''); ?>">
												<a class="page-link" href="<?php print $this->url('/users/' . $i, $query) ?>">
													<?php print $i ?>
												</a>
											</li>
											<?php
									...
									<li class="page-item <?php print ($current >= $pages ? 'disabled' : ''); ?>">
										<a class="page-link" href="<?php print $this->url('/users/' . ($current + 1), $query); ?>" aria-label="Weiter">
